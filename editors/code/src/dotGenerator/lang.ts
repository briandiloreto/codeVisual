...
export class DefaultLang implements Language {
  ...
  fileRepr(file: FileOutline): TableNode {
    // Create sections (table rows) for each function-like symbol in the file
  ...
  symbolRepr(fileId: number, symbol: vscode.DocumentSymbol): Cell {
    // Create a cell to represent the function symbol, creating child cells for each child interface and child function
